<x-app-layout>
    <div class="flex">
        ...
        <div class="flex-1 max-w-5xl mx-auto py-10 px-6">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Yeni Teknik Data Ekle (İş No: {{ $isler->is_no }})</h2>

            <form method="POST" action="{{ route('isler.teknikdata.store', $isler->id) }}" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
                @csrf
                ...
                <fieldset class="border border-gray-300 p-4 rounded">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="parca_gorselligi"
                            ...
                            <option value="evet" {{ old('parca_gorselligi') == 'evet' ? 'selected' : '' }}>Evet</option>
                            <option value="hayir" {{ old('parca_gorselligi') == 'hayir' ? 'selected' : '' }}>Hayır</option>
                        </select>
                    </div>
                </fieldset>
